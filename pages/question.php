...
require_once QA_PLUGIN_DIR . 'q2a-groups/pages/question-view.php';
require_once QA_INCLUDE_DIR . 'app/updates.php';

$questionid = $page;
$cookieid = qa_request_part(3);
...
if (!isset($questionData)) {
	$questionData = qa_db_select_with_pending(
		db_full_post_selectspec($userid, $questionid),
		db_full_child_posts_selectspec($userid, $questionid),
		db_full_a_child_posts_selectspec($userid, $questionid),
		db_post_parent_q_selectspec($questionid),
		db_post_close_post_selectspec($questionid),
		db_post_duplicates_selectspec($questionid),
		db_post_meta_selectspec($questionid, 'qa_q_extra'),
...
if (isset($question)) {
	$q_request = 'groups/' . $groupid . '/' . qa_q_request($questionid, $question['title']);
...
if ($formtype == 'q_edit') { // ...in edit mode
	$qa_content['title'] = $grouptitle . '; <br>' . qa_lang_html($question['editable'] ? 'question/edit_q_title' :
	...
	$qa_content['title'] = $grouptitle . '; <br>' . $qa_content['q_view']['title'];
