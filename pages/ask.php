...
if (!defined('QA_VERSION')) { // don't allow this page to be requested directly from browser
	...
	if (qa_clicked('doask')) {
		require_once QA_PLUGIN_DIR . 'q2a-groups/app/post-create.php';
		...
		if (!qa_check_form_security_code('ask', qa_post_text('code'))) {
			...
			if (empty($errors)) {
				...
				$questionid = question_create($groupid, $followanswer, $userid, qa_get_logged_in_handle(), $cookieid,
				...
				qa_redirect('groups/' . $groupid . '/' . qa_q_request($questionid, $in['title'])); // our work is done here
			}
		}
	}
	...
	$qa_content['title'] = $grouptitle . '; <br>' . 
		qa_lang_html(isset($followanswer) ? 'question/ask_follow_title' : 'question/ask_title');
